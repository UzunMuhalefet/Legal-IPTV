...
def get_page_cameras(start):
    ...
    r = requests.get('https://www.511la.org/List/GetData/Cameras', params=params)
    cameras = r.json()["data"]
    for cam in cameras:
...
def get_all_cameras():
    all_items = []
    r = requests.get('https://www.511la.org/List/GetData/Cameras')
...
def main():
    data = [
        {
            "name": "Louisana - Traffic Cameras",
            "episodes": get_all_cameras()
        }
    ]
    f = open("www-511la-org.json", "w+")
    json.dump(data, f, ensure_ascii=False, indent=4)
    create_single_m3u("../../lists/cameras/countries/us/traffic", data, "la")
